...
class EXU extends Module{
    ...
    io.data.data_sram_we := MuxCase(Fill(4, 0.U), Seq(
        (id_ex_memWe === 1.U && exu_valid && id_ex_csrExcp === 0.U && !io.mem_stop && id_ex_wordType === W) -> Fill(4, 1.U),
        (id_ex_memWe === 1.U && exu_valid && id_ex_csrExcp === 0.U && !io.mem_stop && id_ex_wordType === H) -> Mux(u_alu.io.aluRes(1, 0) === 0.U, "b0011".U, "b1100".U),
        (id_ex_memWe === 1.U && exu_valid && id_ex_csrExcp === 0.U && !io.mem_stop && id_ex_wordType === B) -> (1.U << u_alu.io.aluRes(1, 0))(3, 0)
    ))
    ...
    io.out_csr.excp := id_ex_csrExcp
    io.out_csr.ecode := id_ex_csrEcode
    io.out_csr.esubcode := id_ex_csrEsubcode 
